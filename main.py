...
async def start_ping(proxy, token):
    try:
        last_daily_called = None
        while True:
            today = datetime.date.today()
            if last_daily_called != today:
                asyncio.create_task(daily(proxy=proxy, token=token))
                last_daily_called = today
...
async def daily(proxy, token):
    global last_ping_time, RETRIES, status_connect

    try:
        await call_api("https://api.nodepay.ai/api/mission/complete-mission?", {
            "mission_id": "1"
        }, proxy, token)
        await call_api("https://api.nodepay.ai/api/mission/complete-mission?", {
            "mission_id": "5"
